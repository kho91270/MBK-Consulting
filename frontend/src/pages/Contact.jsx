              ...
              <div className="flex flex-col sm:flex-row gap-4">
                ...
                >
                  <div className="flex items-center justify-center gap-3">
                    ...
                    <span className="font-medium">{language === 'fr' ? 'Réserver un meeting' : 'Book a Meeting'}</span>
                  </div>
          ...
          <div>
            {status === 'success' ? (
              <div className="flex flex-col items-center justify-center h-full py-20 text-center space-y-6">
                ...
                <h3 className="text-2xl font-serif italic">{language === 'fr' ? 'Message envoyé' : 'Message sent'}.</h3>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <input type="text" name="name" required value={formData.name} onChange={handleChange} placeholder={content.form?.name || (language === 'fr' ? 'Nom' : 'Name')} className="w-full px-4 py-3 bg-transparent border border-gray-800 focus:border-gray-600 outline-none transition-colors text-white placeholder-gray-600" />
                <input type="email" name="email" required value={formData.email} onChange={handleChange} placeholder={content.form?.email || 'Email'} className="w-full px-4 py-3 bg-transparent border border-gray-800 focus:border-gray-600 outline-none transition-colors text-white placeholder-gray-600" />
                <textarea name="message" required rows={6} value={formData.message} onChange={handleChange} placeholder={content.form?.message || 'Message'} className="w-full px-4 py-3 bg-transparent border border-gray-800 focus:border-gray-600 outline-none transition-colors resize-none text-white placeholder-gray-600"></textarea>
                {status === 'error' && (
                ...
                <button type="submit" disabled={status === 'sending'} className="w-full py-4 bg-white text-black font-semibold hover:bg-gray-200 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-3">
                  {status === 'sending' ? (
                    <>
